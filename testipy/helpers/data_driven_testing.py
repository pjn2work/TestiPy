...
class DDTMethods(DataReader):
    ...
    def run(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager, tag_name: str, scenario_name: str = "", run_mode: RunMode = None):
        if scenario_name:
            if run_mode is None:
            ...
            if run_mode is RunMode.SCENARIOS_AS_TESTS__USECASES_AS_TESTSTEPS:
                self.run_scenario_as_test__usecases_as_teststeps(sd=sd, ma=ma, rm=rm, tag_name=tag_name, scenario_name=scenario_name, add_test_usecase=True)
            elif run_mode is RunMode.SCENARIO__USECASES_AS_TESTS:
                self.run_scenario__usecases_as_tests(sd=sd, ma=ma, rm=rm, tag_name=tag_name, scenario_name=scenario_name)
            else:
            ...
            if (run_mode is None or
                ...
                self.run_tag(sd=sd, ma=ma, rm=rm, tag_name=tag_name, run_mode=run_mode)
            else:
    ...
    def run_tag(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager, tag_name: str, run_mode: RunMode = None):
        ...
        if _run_mode is RunMode.USECASES_AS_TESTS:
            self.run_usecases_as_tests__without_scenario(sd=sd, ma=ma, rm=rm, tag_name=tag_name)
        elif _run_mode is RunMode.USECASES_AS_TESTSTEPS:
            self.run_usecases_as_teststeps__without_scenario(sd=sd, ma=ma, rm=rm, tag_name=tag_name)
        else:
    ...
    def run_scenario_as_test__usecases_as_teststeps(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager,
                                                    tag_name: str,
                                                    scenario_name: str,
        ...
        current_test = rm.startTest(sd, ma, usecase=usecase_name, description=description)
    ...
    def run_scenario__usecases_as_tests(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager,
                                        tag_name: str,
                                        scenario_name: str):
        _, usecases = self.get_scenarios_or_usecases(tag_name=tag_name, scenario_name=scenario_name)
        for usecase_name, usecase in usecases.items():
            ...
            current_test = rm.startTest(sd, ma, usecase=usecase_name, description=usecase.get("_description_", ""))
    ...
    def run_usecases_as_tests__without_scenario(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager, tag_name: str):
    ...
    def run_usecases_as_teststeps__without_scenario(self, sd: SuiteDetails, ma: TestMethodAttr, rm: ReportManager, tag_name: str):
        _, usecases = self.get_scenarios_or_usecases(tag_name=tag_name)

        # the use-case (test) may have as description
        current_test = rm.startTest(sd, ma, usecase=tag_name, description=usecases.get("_description_", ""))
        if rm.has_ap_flag("--norun"):
            rm.testSkipped(current_test, "--norun")
        else:
            _, failed_usecase = self._run_usecases_as_teststeps(rm, current_test, usecases)
