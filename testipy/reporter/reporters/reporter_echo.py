...
def show_totals(state_counter):
    print(
        tabulate(
            state_counter.get_summary_per_state(),
...
class ReporterEcho(ReportInterface):
    ...
    def _teardown_(self, end_state: str):
        ...
        print("")
        print(
            textdecor.color_line(
                tabulate(
                    dfm.reduce_datetime(
                        dfm.get_test_ros_summary(df).drop(columns=["End time"])
                    ),
                    headers="keys",
                    tablefmt="simple",
                    showindex=False,
        ...
        print(
            tabulate(
                dfm.reduce_datetime(dfm.get_test_dummies(df)),
                headers="keys",
                tablefmt="simple",
                showindex=False,
            )
        ...
        print(
            textdecor.color_line(
                tabulate(
                    dfm.reduce_datetime(dfm.get_suite_state_summary(df)),
                    headers="keys",
                    tablefmt="simple",
                    showindex=False,
                )
    ...
    def test_info(
        self, current_test: TestDetails, info: str, level: str, attachment: Dict = None
    ):
    ...
    def end_test(
        ...
        print(
            f"> {test_full_name} - {end_state} - took {format_duration(test_duration)} - reason: {end_reason} <".center(
                _line_size + len(end_state) - len(ending_state), "-"
            )
        )
        print("-" * _line_size)
