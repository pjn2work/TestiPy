...
class ReporterExcel(ReportInterface):
    ...
    def __init__(self, rm: ReportManager, sa: StartArguments):
        ...
        self.writer = pd.ExcelWriter(
            self._fpn, engine="xlsxwriter", datetime_format="yyyy-mm-dd hh:mm:ss.000"
        )
    ...
    def _create_summarys(self, df):
        # write summarys
        dfm.get_levels_state_summary(df).to_excel(
            self.writer, index=False, header=True, sheet_name="#Summary"
        )
        dfm.get_package_dummies(df).to_excel(
            self.writer, index=False, header=True, sheet_name="#PackageSummary"
        )
        dfm.get_suite_dummies(df).to_excel(
            self.writer, index=False, header=True, sheet_name="#SuiteSummary"
        )
        dfm.get_test_dummies(df).to_excel(
            self.writer, index=False, header=True, sheet_name="#TestSummary"
        )
        dfm.get_test_ros_summary(df).to_excel(
            self.writer, index=False, header=True, sheet_name="#TestROS"
        )
        ...
        for package_name in self._df_step_counters["Package"].unique():
            ...
            df_steps = self._df_step_counters[
                self._df_step_counters["Package"] == package_name
            ]
            df_steps.to_excel(
                self.writer, index=False, header=True, sheet_name=sheet_name
            )
