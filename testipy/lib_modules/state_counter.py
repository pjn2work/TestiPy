...
if __name__ == "__main__":
    from testipy.configs.enums_data import STATE_PASSED, STATE_FAILED, STATE_SKIPPED
    ...
    sleep(0.1)
    sc.inc_state(STATE_PASSED, "ok", "first step", 2)
    sleep(0.2)
    try:
        _ = 1/0
    except Exception as ex:
        sc.inc_state(STATE_FAILED, reason_of_state="timeout", exc_value=ex)
    sleep(0.3)
    sc.inc_state(STATE_PASSED, "ok", qty=8)
    sleep(0.4)
    sc.inc_state(STATE_SKIPPED, "no args", qty=2)
    sc.inc_state(STATE_SKIPPED, description="step 2", qty=2)
    sc.inc_state(STATE_SKIPPED, "new skip", qty=2)
    sc["NewState"] = 19

    print("str()          =", str(sc))
    print("repr()         =", sc.__repr__())
    print("Total qty      =", sc.get_total())
    print("PASSED inside? =", STATE_PASSED in sc)
    print("Total PASSED   =", sc[STATE_PASSED])
    print("% PASSED       =", sc.get_state_percentage(STATE_PASSED))
    print("% NewState     =", sc.get_state_percentage("NewState"))
    print("RoS            =", sc.get_reasons_of_state("NewState"))

    print("\nLast Consecutive was {} with {} incremets.".format(sc.get_last_state(), sc.get_last_consecutive_qty()))
    for row in sc.get_totals_percentage(" \t\t L__> "):
        print(row)
    print("Reasons of SKIPPED     =", sc.get_reasons_of_state(STATE_SKIPPED))
    print("Reasons of states      =", sc.get_reasons_of_states())

    print("\nTimelaps list:")
    for row in sc.get_timed_laps():
        print(" ", row)
    print("Last state change time =", sc.get_timed_last_lap())
    print("Total Time             =", sc.get_timed_total_elapsed())

    print("\nIterating StateCounter:")
    for x, y in sc.items():
        print("\t", x, y)

    sc2 = StateCounter()
    sc2.inc_state(STATE_PASSED)
    sc2.inc_states(sc)

    print("="*100)
    print("Full list:")
    for row in sc.get_summary_per_state():
        print(" ", row)
    print("="*100)

    sc.reset()
    print("\nIterating StateCounter after reset:")
    for x in sc:
        print("\t", x, sc[x])

    print("\nSC2 added from sc1 resume =", sc2)
    print("\nSC2 added from sc1        =", sc2.get_reasons_of_states())
    for row in sc2.get_timed_laps():
        print("Timelaps list =", str(row[-1]), row[:-1])
